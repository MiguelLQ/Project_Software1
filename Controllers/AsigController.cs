...
namespace albanaPlayaEst.Controllers
{
    public class AsigController : Controller
    {
        ...
        public IActionResult Index(string descripcion)
        {
            ...
            // Asignar las descripciones al ViewBag para la vista
            ViewBag.Descripciones = new SelectList(descripciones);

            // Asignar la descripción seleccionada al ViewBag o ViewData
            ViewBag.SelectedDescripcion = descripcion;

            // Obtener los espacios, filtrando por la descripción si se ha seleccionado alguna
            var espacios = _context.Espacios
                .Where(e => string.IsNullOrEmpty(descripcion) || e.Descr_esp.Contains(descripcion)) // Filtrar por descripción si se seleccionó
        ...
        public IActionResult FiltrarPorDescripcion(string descripcion)
        {
            // Recuperar las descripciones únicas de los espacios
            var descripciones = _context.Espacios
                .Select(e => e.Descr_esp)
                .Distinct()
                .ToList();

            // Si no hay descripciones, asignar una lista vacía
            if (descripciones == null || !descripciones.Any())
            {
                descripciones = new List<string>();  // Evita nulos
            }

            // Asignar descripciones al ViewBag para la vista
            ViewBag.Descripciones = new SelectList(descripciones);

            // Asignar la descripción seleccionada para que se mantenga en el filtro
            ViewBag.SelectedDescripcion = descripcion;

            // Filtrar los espacios por la descripción seleccionada
            var espaciosFiltrados = _context.Espacios
                .Where(e => string.IsNullOrEmpty(descripcion) || e.Descr_esp.Contains(descripcion))  // Filtrar por descripción
                .Include(e => e.Registros)  // Incluir registros de espacios
                .ToList();

            // Mapear los espacios filtrados a DTOs
            var espacioDtos = espaciosFiltrados.Select(e => new AsignarVDto
            {
                Cod_esp = e.Cod_esp,
                Descr_esp = e.Descr_esp,
                Estad_esp = e.Estad_esp,
                CodReg = e.Registros?.FirstOrDefault()?.CodReg
            }).ToList();

            return View("Index", espacioDtos);  // Retornar vista con los espacios filtrados
        }
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var nuevoPago = new Pago
                    {
                        ...
                        FechaInicio = asignarVDto.FechaInicio,
                ...
                catch (Exception ex)
                {
                    ...
                    return Json(new { success = false, error = "Hubo un problema al asignar el vehículo y procesar el pago.", details = ex.Message });
                }
            }
        ...
        {
            // Consulta para obtener todos los registros de espacios ocupados
            var registrosOcupados = _context.Registros
                ...
                .Where(r => r.CodEspNavigation.Estad_esp == true) // Solo los espacios ocupados
                .ToList(); // Traer todos los registros primero
            ...
            {
                // Si no hay espacios ocupados, puedes agregar un mensaje a la vista
                ViewBag.Mensaje = "No hay espacios ocupados actualmente.";
            }
